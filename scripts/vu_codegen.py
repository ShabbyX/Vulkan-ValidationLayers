...
def removeOneArrayLevel(t):
    # Remove one array level from the type.  For example:
    #
    #     type * -> type
    #     type * const * -> type *
...
def getAPIMemberInfo(member):
    ...
    # Pointer level is taken as all that is between the type and the name.  For example:
    #
    #     type name -> ''
    #     type* name -> '*'
    #     type* const* name -> '* const*'
    typeStart = decl.find(member.type)
    assert(typeStart != -1)
    pointerLevelStart = typeStart + len(member.type)

    pointerLevelEnd = decl.find(member.name)
    assert(pointerLevelEnd != -1)

    pointerLevel = decl[pointerLevelStart:pointerLevelEnd].strip()

    # Add pointer levels for each fixed size array dimension
    pointerLevel += '*' * len(member.fixedSizeArray)

    return member.name, Type(member.type, pointerLevel, arrayLen)

def getAPIParams(vk, api):
    members = []

    if api in vk.structs:
        members = vk.structs[api].members
    elif api in vk.commands:
        members = vk.commands[api].params
    else:
...
def getAPIParamInfo(vk, api, symbol):
    ...
    members = getAPIParams(vk, api)
    for member in members:
        if symbol == member.name:
...
class VuCodegen(ast.NodeVisitor):
    def __init__(self, vk, api, members, vuids, originalVus, variableIdMap):
        self.api = api
    ...
    def visit_If(self, node):
        ...
        if len(node.orelse) > 0:
            result += ['else']
            result += self.visitBody(node.orelse)
    ...
    def visit_UnaryOp(self, node):
        ...
        no_parens = (isinstance(node.op, ast.USub) and
    ...
    def visit_Call(self, node):
        ...
        if funcname == 'is_ext_enabled':
            return ['IsExtEnabled(device_extensions.', node.args[0].id.lower(), ')'], Type('bool')
        if funcname == 'is_feature_enabled':
            return ['enabled_features.', node.args[0].id], Type('bool')
        ...
        if funcname in ['has_pnext', 'is_version', 'is_feature_enabled', 'externally_synchronized', 'has_bit', 'any', 'none', 'valid']:
            resultType = Type('bool')
        elif funcname == 'pnext':
    ...
    def visit_Attribute(self, node):
        ...
        _, memberType = getAPIParamInfo(self.vk, objType.typeStr, node.attr)
...
def generateInit(membersInfo):
    ...
    for name, objType in membersInfo:
...
def generateValidation(vk, api, vus):
    ...
    memberNames = [name for name, _ in membersInfo]

    generator = VuCodegen(vk, api, memberNames, vuids, originalVus, variableIdMap)
