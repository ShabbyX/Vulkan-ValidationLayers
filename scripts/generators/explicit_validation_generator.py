...
class ExplicitValidationOutputGenerator(BaseGenerator):
    ...
    def makeCallStructValidation(self, structName, expr, parentObjects, indent, dont_recurse_into_pnext = False):
        ...
        for member in self.vk.structs[structName].members:
            # When validating a pnext chain, don't let the struct validation functions recurse into the pnext chain; the rest of the chain will be processed right after.
            if dont_recurse_into_pnext and member.name == 'pNext':
                out += [separator, 'nullptr']
                continue
...
"""]
        ...
        for struct in self.vk.structs.values():
            ...
            out += self.makeCallStructValidation(struct.name,
                                                 ...
                                                 '_parentObjects',
                                                 indent + 1, dont_recurse_into_pnext = True)
...
"""]
    ...
    def makeCallHelper(self, api, members, apiType):
        out = self.makeFunctionName(api, apiType)
        separator = '('
        ...
        if api in APIS_WITH_ADDITIONAL_STATE_ARG:
            out += [separator, ADDITIONAL_STATE_ARG]
    ...
    def makeAliasCall(self, alias, members, apiType):
        ...
        out += self.makeCallHelper(alias, members, apiType)
        out.append('\n;')
        return out

    def makeStateTrackerCall(self, api, members, apiType):
        out = ['ValidationStateTracker::']
        out += self.makeCallHelper(api, members, apiType)
        return out
    ...
    def generateImpl(self, api, members, alias, apiType):
        ...
        if alias:
            ...
            # Some functions need to forward to the state tracker first.
            # TODO: instead of having every layer track the same state, make
            # the state tracker object separate from the layers, call it, then
            # pass the same state into all layers.
            out += [self.makeIndent(1), 'bool skip = ']
            if api in APIS_WITH_ADDITIONAL_STATE_ARG:
                out += self.makeStateTrackerCall(api, members, apiType)
            else:
                out.append('false')
            out.append(';\n')
