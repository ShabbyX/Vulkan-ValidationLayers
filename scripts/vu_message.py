...
class VuPrintfStyler:
    ...
    def verifyIdentical(self, originalAST, formatted):
        ...
        def turnCommentIntoCall(line):
            if line.lstrip()[0] == '#':
                hashIndex = line.index('#')
                return ' ' * hashIndex + '__comment("' + line[hashIndex + 2:] + '")'
            return line

        # Turn comments back into functions that show up in the AST
        toVerify = '\n'.join(turnCommentIntoCall(line) for line in toVerify.splitlines())
    ...
    def getNameSymbol(self, node):
        ...
        specifier, entityIsHandle = self.getPrintfConversionSpecifier(exprType)
        if specifier == None:
            result = [noIdSymbol]
        ...
        arg, obj = self.getPrintfArg(expr, exprType, entityIsHandle)
    ...
    def onEndScope(self, scope):
        ...
        if not self.requireVisited:
            ...
            self.objects = self.objects[:objectsStart]
    ...
    def getPrintfConversionSpecifier(self, exprType):
        ...
        vk = self.codegen.vk
        if typeStr in vk.handles:
            return '%s', True

        if typeStr in vk.bitmasks:
            return '%' + STR_END + ' PRIx' + str(vk.bitmasks[typeStr].bitWidth) + ' ' + STR_BEGIN, False

        # TODO: any other category that may need special attention

        if typeStr == 'uint8_t':
            return '%' + STR_END + ' PRIu8 ' + STR_BEGIN, False
        if typeStr == 'int8_t':
            return '%' + STR_END + ' PRId8 ' + STR_BEGIN, False
        if typeStr == 'uint16_t':
            return '%' + STR_END + ' PRIu16 ' + STR_BEGIN, False
        if typeStr == 'int16_t':
            return '%' + STR_END + ' PRId16 ' + STR_BEGIN, False
        if typeStr  == 'uint32_t':
            return '%' + STR_END + ' PRIu32 ' + STR_BEGIN, False
        if typeStr == 'int32_t':
            return '%' + STR_END + ' PRId32 ' + STR_BEGIN, False
        if typeStr in ['uint64_t', 'VkDeviceSize', 'VkDeviceAddress']:
            return '%' + STR_END + ' PRIu64 ' + STR_BEGIN, False
        if typeStr == 'int64_t':
            return '%' + STR_END + ' PRId64 ' + STR_BEGIN, False
        if typeStr == 'float':
            return '%f', False
        if typeStr == 'double':
            return '%lf', False
        ...
        if typeStr in ['VkSampleMask', 'VkFlags']:
            return '%#' + STR_END + ' PRIx32 ' + STR_BEGIN, False
    ...
    def getPrintfArg(self, expr, exprType, entityIsHandle):
        expr = ''.join(expr)
