...
static std::string debug_log_format_msg(const debug_report_data *debug_data, VkFlags msg_flags, std::string_view vuid_text, const char *format, va_list argptr) {
...
static std::string debug_log_get_link(std::string_view vuid_text, const std::string &spec_type) {
...
VKAPI_ATTR bool LogFormattedMsg(const debug_report_data *debug_data, VkFlags msg_flags, const LogObjectList &objects, std::string_view vuid_text, const char *format, va_list argptr) {
    ...
    // TODO: make autogen output this somewhere usable
    ...
    return debug_log_msg(debug_data, msg_flags, objects, "Validation", message.c_str(), vuid_text.data());
}
