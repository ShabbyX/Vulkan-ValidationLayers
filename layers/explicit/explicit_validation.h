...
class ExplicitValidation : public ValidationStateTracker {
  public:
    ...
    {
        return vku::FindStructInPNextChain<S>(pNext) != nullptr;
    }

    template <typename S>
    const S *Builtin_pnext(const void *pNext) const
    {
        return vku::FindStructInPNextChain<S>(pNext);
    }

    bool Builtin_is_version(uint32_t major, uint32_t minor) const
    {
        return api_version >= VK_MAKE_API_VERSION(0, major, minor, 0);
    }

    // Builtin_is_ext_enabled is not defined, as the generator directly translates to IsExtEnabled.
    // Builtin_is_feature_enabled is not defined, as the generator directly translates to enabled_features.foo.
...
#include "generated/explicit_validation_decl.h"
};
